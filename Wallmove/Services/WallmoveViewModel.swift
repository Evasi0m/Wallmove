...
final class WallmoveViewModel: ObservableObject {
    ...
    @Published private(set) var previewSurfaceID = UUID()
    ...
    func importWallpapers() {
        ...
        panel.message = "Choose one or more .mp4 or .mov files to copy into Wallmove."
    ...
    func setPreviewPresentation(isVisible: Bool) {
    ...
    func rebindPreviewSurface() {
        previewSurfaceID = UUID()
    ...
    private func applyEngineConfiguration() {
        wallpaperEngine.configure(desktopURL: library.activeWallpaperURL())
    }

    private func keepDashboardVisible() {
